...
> 生成日期：2026-09-20 ｜ 设计师：颜好看 ｜ 基于：《01-需求文档（专业版）》V1.0 + 《03-技术方案》V1.0 + 《04-界面设计说明》V1.0（编号文档为仓库外交接存档，本文件是它们在仓库内的落地契约）
> 三轴刻度：Variance=3 / Motion=3 / Density=4
> 定位：本文件是全项目**设计契约源文件**（10 节标准）。页面清单与组件清单见 `spec-分片归档.md`；Token 定义见 `frontend/src/styles/design-tokens.css` / `docs/design-tokens.json`。
...
| 2026-09-24 | 新增第 10 节「文案与语气标准」；全站用户可见文案去黑话（吃书→前后矛盾等） | 目标用户是零基础写作者，要专业感、去 AI 味 | 文案层 |

---

## 10. Copy & Voice（文案与语气标准）

目标用户是**零基础写作者**，同时要有**专业工具的质感**——两条同时成立，靠的是"清楚"而非"卖萌"。

**声音定位**：像一位耐心的资深文字编辑，不像聊天机器人，也不像客服话术。
- 平实、具体、可执行；一句能说清不用两句。
- 解释"为什么"，不堆功能名：❌"支持记忆金字塔" → ✅"写到后面，前面埋的伏笔它还记得"。
- 结果导向的肯定句，少用感叹号、省略号、波浪号与语气词（"哦/啦/哈/别慌"）。

**禁用清单（AI 味 / 卡通味）**
- 客服腔与过度热情："亲～""太棒啦""一起开启创作之旅吧！"。
- 空泛形容词堆砌（"强大""智能""海量"）替代具体能力说明。
- emoji 作装饰或功能图标（与第 7 节一致）。
- 拟人化卖萌口吻；工具是"在场帮手"，不是"性格角色"。
- 技术黑话直接抛给用户：字段 / 枚举 / JSON / Token / slug / 记忆包等只出现在代码与本文档，界面用大白话。

**行业术语要翻译**：网文圈黑话对新手是门槛——"吃书"→"前后矛盾"、"金手指"保留但首次出现给一句解释、"钩子"→"让读者想往下看的悬念"。术语只作标签括注（如"一致性审校（防前后矛盾）"），不裸用。

**降门槛三件套**（每个界面元素自带）
1. 标题：这一步叫什么（名词短语，≤8 字）。
2. 说明：一句话说清"做它有什么用"，必要时给例子（"例如：番茄男频，爱看扮猪吃虎的下班读者"）。
3. 出口：明确下一步或"可跳过"，绝不用必填把人堵死（红线 3：不配模型也能写作）。

**错误与空态文案**：说清"发生了什么 + 现在能怎么办"，不甩锅、不吓人、不粉饰。空态给一条主行动按钮；错误就地显示 + 重试，不弹 alert（第 7 节）。

**一致性**：同一概念全站同一个词（"作品"不混用"书/项目"、"章节"不混用"小节"）；动作按钮用"动词+宾语"（"保存简介"而非"确定"）；数字统一 `tabular` 等宽。
