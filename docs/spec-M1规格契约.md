...
**配套文档**（不重复内容，只引用）：
- `docs/spec-范围章节.md` — 第 2/3/4 章的完整表格（PM 许清楚）
- `docs/spec-技术章节.md` — 第 4/5/6/11 章的完整表格（架构师高见远）
- `docs/spec-设计与页面章节.md` — 第 7/8 章的完整表格（设计师颜好看）
...
共 **12 条功能**（11 项需求 R1–R7、R11、R15–R17 + 1 项基础设施），完整表格与 RICE 评分见 `spec-范围章节.md` 第 2 章。
...
完整表格（21 条，含原因与"何时考虑"）见 `spec-范围章节.md` 第 3 章。**开发中如有人提出以下功能，直接拒绝，走变更流程。**
...
**总计 36 个 path**（`06-openapi.yaml` 原有 33 个 + 补齐 2 个遗漏端点 + 新增 1 个能力探测端点）。完整清单见 `spec-技术章节.md` 第 5 章。
...
> 写法为「现象 → 规避」，目的在事前避开，而非事后返工。完整版见 `spec-技术章节.md` 第 11 章。
