...
项目级 P0 规则：**禁止 emoji 作为功能图标**；Spec 必须锁定**一套**描边（stroke）风格 SVG 图标库，全项目统一、不混用。设计契约《DESIGN.md》§4 与《spec-分片归档》§8.3/§10 已把图标库列为**占位（`--icon.library = "PENDING"`）**，明确“待架构师锁定后替换”，并要求所有图标经 `Icon` 组件**单一入口**渲染，替换时只改一处。
...
- 与设计契约《DESIGN.md》§4、《spec-分片归档》§8.3/§10 一一对应（替换 PENDING 占位）。
